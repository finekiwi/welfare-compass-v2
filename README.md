...
> 자연어 챗봇 경로는 검색/에이전트 파이프라인 중심으로 구현을 진행했고, 프론트 연동과 실데이터 E2E 경로는 순차적으로 고도화 중입니다.
...
- 프론트 챗봇 경로와 LangGraph 기반 응답의 실연동은 현재 순차적으로 고도화 중입니다.
...
> 프론트의 챗봇 요청은 현재 Backend Chat API를 통해 처리되며, LangGraph 기반 응답 경로는 연동 진행 중입니다.
